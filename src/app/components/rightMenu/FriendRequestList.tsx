...
const FriendRequestList = ({requests}:{requests: RequestWithUser[]}) =>{
    ...
    const decline = async (requestId: number, userId: string ) =>{
        ...
        const [optimisticRequestState, removeOptimisticRequest] = useOptimistic(requestState, (state, value: number)=> state.filter((req) => req.id !==value)  );
        ...
        <div className="">
            { optimisticRequestState.map((request) =>(
                ...
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-semibold ">{(request.sender.name && request.sender.surname)? request.sender.name + " " + request.sender.surname: request.sender.username}</span>
                </div>
             {/* accept/reject */}
